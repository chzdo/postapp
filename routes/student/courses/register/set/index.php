...
try {
    ...
    if(!$prog->isCurrent($postdata['session_id']) || $prog->isClosed($postdata['session_id'] ) ){
     ...
     if($session_flag){
          ...
          $info['id'] = $stu->info['student_id'];
          $info['session'] = $postdata['session_id'];
            if(!$tokenizer->isReason($info,'register')){
                echo sendJson(array('code'=>0, 'message'=>'Reason Match failed', 'payload'=>null ));
                return;
            }
         
     }
     $r = $dept->getMaxLoad($stu->info['dept_id'],$postdata['semester_id'] , $stu->info['prog_id']);
 

     if($r['code']==0){
        echo sendJson(array('code'=>0, 'message'=>'Credit load maxmium not found', 'payload'=>null ));
        return;
     }
   
$load = 0;
     foreach($postdata['list'] as $key=>$list){
         $load += $list['course_load'];
     }

     if((int)$load > (int)$r['payload']['unit_load']){
        echo sendJson(array('code'=>0, 'message'=>'maximium credit load exceeded', 'payload'=>null ));
        return;
     }
