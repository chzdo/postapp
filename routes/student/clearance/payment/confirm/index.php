...
if (!$type->VerifyOrderID($id,$student->info['appd_id'],$order_id)){
    echo sendJson(array('code'=>0, 'message'=>'This OrderID does not belong to you ', 'payload'=>null));
return;
}
...
if ($session->isClosed($type->pay_info['session']) || ! $session->isCurrent($type->pay_info['session'])){
    echo sendJson(array('code'=>0, 'message'=>'Session does not exist ', 'payload'=>null));
return;
}
...
$verify = $type->verifyOrderPayment();

if($verify == null){
    echo sendJson(array('code'=>0, 'message'=>"Network Error", 'payload'=>null));
    return;
}
...
if($clear){
    ...
    if($type->pay_info['clear_type'] == SCHOOL_FEE && $student_type){
       ...
       $resp = $student->createMatricNumber($client,$email);
       if($resp['code']==0){
...
$n->notify(EMAIL_CODE_PAY,$info);
    ...
    $type->pay_info['redirect'] = REDIRECT.$type->pay_info['session'].$type->pay_info['url'];
