...
try {
    ...
    if(!count($postdata['list']) > 0){
        echo sendJson(array('code' => 0, 'message' => "No Course Selected", 'payload' => null));
        return;
    }
    $response = $dept->setCourse($postdata['dept_id'],$postdata['list'],$postdata['semester_id'],$postdata['prog_id'],$course);
    if ($response['code'] == 0) {
    ...
    $response = $dept->getCourse($postdata['dept_id'],$postdata['semester_id'],$postdata['prog_id']);
    ...
    $response = $course->allActiveWithLoad($postdata['dept_id'],$postdata['semester_id']);
    if ($response['code'] == 0) {
        echo sendJson(array('code' => 0, 'message' => $response['message'], 'payload' => null));
        return;
 }
...
(new Logs($db))->eventLog($decoded->data->id, EVENTS['setcourses_d'].'  with  name '.$dept->name .'  '.$prog->name);
