...
try {
    ...
    $response = $faculty->Update($postdata['id'],$postdata['name'], $postdata['description'], $decoded->data->id);
    if ($response['code'] == 1) {
        (new Logs($db))->eventLog($decoded->data->id,EVENTS['update_f'].'  with  name '.$faculty->name);
