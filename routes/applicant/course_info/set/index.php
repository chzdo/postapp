...
if (!isset($postdata['programme_id'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['faculty_id'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if (!isset($postdata['options_id'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['research_topic'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
try {
...
extract(get_object_vars($decoded->data));
 $n = new N($client,$db);
 ...
 $info = array(
     ...
     "option"=> $options,
     "cron"=>false,
     "type"=>EMAIL_CODE_APP_CREATE_COURSE
 );
 $n->notify(EMAIL_CODE_APP_CREATE_COURSE,$info);
 $a = $app->getSideSummary();

 
 $b = $app->getSummary();

  echo sendJson(array("code"=>1 , "message"=>"Course Found","payload"=>array('course'=>$app->course, 'summary'=>array("side"=>$a,"main"=>$b))));
  return;
} catch (Exception $e) {
