...
if($email == false){
 ...
 if($app->app['appd_id'] == '' ||$app->app['appd_id'] == null )
 {
    echo sendJson(array('code' => 0, 'message' => "No Application Yet", 'payload' => ["redirect"=>true]));
    return;
 }
 ...
 $b = $app->getSummary();
  ...
  echo sendJson(array("code"=>1 , "message"=>"Course Found","payload"=>array("side"=>$a,"main"=>$b)));
  return;
} catch (Exception $e) {
