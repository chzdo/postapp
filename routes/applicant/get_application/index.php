...
if($email == false){
 ...
 $r['current_session'] = $session->checkCurrentAdmission($postdata['session']);

 $r['session_closed'] = @$session->info['app_state'] == 1? false: true;




$postdata['email'] = $email;
 $resp = $app->get_application($postdata);

if((int)$resp['payload']['app_count'] > 0){
    $resp['payload']['app_summary']['session'] = $session->name;
}
 
$r = array_merge($r,$resp['payload']);


  echo sendJson(array("code"=>1 , "message"=>$resp['message'],"payload"=>$r));
  return;
} catch (Exception $e) {
