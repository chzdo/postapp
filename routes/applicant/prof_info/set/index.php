...
if (!isset($postdata['prof_qual'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['award_body'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['year_obtained'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if($email == false){
 ...
 $b = $app->setProf($postdata);
   ...
   $b = $app->getSummary();
  echo sendJson(array("code"=>1 , "message"=>"Certification  Found","payload"=>['prof'=>$app->prof, 'summary'=>array("side"=>$a,"main"=>$b)]));
   return;
} catch (Exception $e) {
