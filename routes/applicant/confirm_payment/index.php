...
if (!$app->VerifyOrderID($postdata)){
    echo sendJson(array('code'=>0, 'message'=>'This RRR does not belong to you ', 'payload'=>null));
return;
}
if($app->payments['status']==1){
    echo sendJson(array('code'=>0, 'message'=>'You have already Verified this payment ', 'payload'=>null));
    return;
}

$payment->pay_info['order_id'] = $postdata['order_id'];
...
extract($app->payments);
$info = array(
    "name"=> $surname.', '.$firstname.' '.$othername,
    "id" => $appd_id == null ?  $email: $appd_id,
    "email"=> $email,
    "rrr" =>$rrr,
    "session" => $s,
    "purpose" =>$purpose,
