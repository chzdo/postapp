...
if (!isset($postdata['other_course'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['prizes'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['pro_prof'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if($email == false){
 ...
 $b = $app->setOther($postdata);
   ...
   $b = $app->getSummary();
  echo sendJson(array("code"=>1 , "message"=>"Other Found","payload"=>['other'=>$app->other, 'summary'=>array("side"=>$a,"main"=>$b)]));
   return;
} catch (Exception $e) {
