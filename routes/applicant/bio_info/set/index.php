...
if (!isset($postdata['postal_address'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if (!isset($postdata['marital_status'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if($email == false){
 ...
 if(!$app->app['status'] == 1){
  ...
  if($app->getBio() && (@$_FILES['passport']['name'] == '' || !isset($_FILES['passport']))){
    
      $file = $app->bio['passport_r'];
  }else{
      $file = $_FILES['passport'];
  }

 
 $a = $app->uploadPassport($file);
 if($a['code']==0){
    echo sendJson($a);
    return;
 }
 $b = $app->setBio($postdata);
   if(!$b){
   ...
   $b = $app->getSummary();
  echo sendJson(array("code"=>1 , "message"=>"Bio Found","payload"=>array("bio"=>$app->bio, "summary"=>array("side"=>$a,"main"=>$b))));
  return;
} catch (Exception $e) {
