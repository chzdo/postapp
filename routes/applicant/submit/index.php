...
if($email == false){
 ...
 $b = $app->Submit($postdata);
   ...
   $n = new N($client,$db);
   $info = array(
       "name"=> $decoded->data->surname.', '.$decoded->data->firstname.' '.$decoded->data->othername,
       "id" => $app->app['appd_id'],
       "email"=> $email,
       "cron"=> false,
       "type"=>EMAIL_CODE_APP_SUBMIT
   );
   $n->notify(EMAIL_CODE_APP_SUBMIT,$info);
   $a = $app->getSideSummary();

 
   $b = $app->getSummary();
  
  echo sendJson(array("code"=>1 , "message"=>"Application Submitted","payload"=>['summary'=>array('side'=>$a, 'main'=> $b)]));
  return;
} catch (Exception $e) {
