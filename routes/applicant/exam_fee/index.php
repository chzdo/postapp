...
try {
  ...
  //  $query_staff = new Query($db_staff);
   ...
   // $app->query_staff = $query_staff;
    ...
    $postdata['type'] = APP_PAYMENT_EXAM;
    $fee = $app->getFee(APP_PAYMENT_EXAM);
    ...
    $payment->info = $fee;
    if ($app->checkPayment($postdata)) {
        if ($app->payment['status'] == 1) {
            $app->payment['name'] = $info['name'];
            $app->payment['phone'] = $info['phone'];
            unset($payment->info['amount']);
            $app->payment['session'] = $session->name;
              echo sendJson(array("code" => 1, "message" => "Payment Generated", "payload" =>['summary'=>array('side'=>$a, 'main'=>$b), 'pay_info'=> array_merge($app->payment,$payment->info)]));
        ...
        if (!EXAM_PAYMENT_ACTIVE) {
            echo sendJson(array('code' => 0, 'message' => "Payments for pre qualifying Exams have not started",));
            return;
        }
        ...
        $pay = array(
            "type" => APP_PAYMENT_EXAM, "amount" => $payment->info['amount'],
    ...
    echo sendJson(array("code" => 1, "message" => "Payment Generated", "payload" =>['summary'=>array('side'=>$a, 'main'=>$b), 'pay_info'=> $info]));
    return;
} catch (Exception $e) {
