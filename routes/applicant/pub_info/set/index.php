...
if (!isset($postdata['publisher'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['issn'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if($email == false){
 ...
 $b = $app->setPub($postdata);
   ...
   $b = $app->getSummary();
  echo sendJson(array("code"=>1 , "message"=>"Publication Found","payload"=>['pub'=>$app->pub, 'summary'=>array("side"=>$a,"main"=>$b)]));
   return;
} catch (Exception $e) {
