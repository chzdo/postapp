...
try {
    ...
    $info['purpose'] = $payment->info['name'] ;
   if ($payment->checkAcceptPayInfo($app->app['appd_id'], $postdata['session'], $pay_type)) {

                      if($payment->pay_info['status']==1){
                        $payment->pay_info['purpose'] = $payment->info['name'] ;
                        $payment->pay_info['name'] = $decoded->data->surname . ', ' . $decoded->data->firstname . ' ' . $decoded->data->othername;
                        $payment->pay_info['email'] = $decoded->data->email;
                        $payment->pay_info['phone'] = $decoded->data->phone;
                       
                        echo sendJson(array('code' => 1, 'message' => "Payment Found", 'payload' => ['pay_info'=> $payment->pay_info, 'clear'=>$clr]));
                        return;
                      }
    ...
    $info['response'] = APP_URL.'/session/'.$postdata['session'].'/confirm/25';
   
    

    echo sendJson(array("code" => 1, "message" => "Payment Generated", "payload" =>['pay_info'=> $info, 'clear'=>$clr]));
    return;
} catch (Exception $e) {
