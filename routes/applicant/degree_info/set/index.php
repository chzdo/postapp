...
if (!isset($postdata['to_date'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['programme'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['cgpa'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['degree_class'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
if (!isset($postdata['qualification'])) {
    echo sendJson(array('code' => 0, 'message' => RESPONSE['invalid'], 'payload' => null));
    return;
}
...
if($email == false){
 ...
 $b = $app->setDegree($postdata);
   ...
   $b = $app->getSummary();
  echo sendJson(array("code"=>1 , "message"=>"Degree Found","payload"=>['degree'=>$app->degree, 'summary'=>array("side"=>$a,"main"=>$b)]));
  return;
} catch (Exception $e) {
