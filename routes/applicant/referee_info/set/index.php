...
if($email == false){
 ...
 $post['email'] = $email;
 $post['session'] = $postdata['session'];
 if(!$app->isAppUser($post)){
    echo sendJson(array('code' => 0, 'message' => "No App found for this session", 'payload' => ["redirect"=>true]));
    return;
 }
  $e = filter_var($postdata['email'],FILTER_VALIDATE_EMAIL);
  if($e == false){
    echo sendJson(array('code' => 0, 'message' => "Invalid Referee email address ", 'payload' => null));
...
if(!preg_match("/^\+[0-9]{1,3}[0-9]{11}$/",$postdata['phone'])){
    echo sendJson(array('code' => 0, 'message' => "Invalid Referee Phone Number", 'payload' => null));
    return;
}
if(!isset($postdata['resend'])){
 $b = $app->setReferee($postdata);
   if($b['code']==0){
    echo sendJson($b);
    return;
   }
   ...
   $info = array("student_id"=> $app->app['appd_id'], 
   "student_name"=> $decoded->data->surname." ".$decoded->data->firstname,
   "name"=>$postdata['name'],
    "email"=>$postdata['email'],
    "type"=>EMAIL_CODE_APP_REFEREE_CREATE , "cron"=>false);
   $notify->notify(EMAIL_CODE_APP_REFEREE_CREATE,$info);
   ...
   $b = $app->getSummary();
  echo sendJson(array("code"=>1 , "message"=>"Email Sent","payload"=>['referee'=>$app->referee, 'summary'=>array("side"=>$a,"main"=>$b)]));
   return;
} catch (Exception $e) {
