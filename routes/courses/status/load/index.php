...
try {
    ...
    if(!$faculty->checkWithLoad($postdata['id'])){
        echo sendJson(array('code' => 0, 'message' => "Course does not exist", 'payload' => null));
        return;
    }
    $response = $faculty->setStatusWithLoad($postdata['id'],$postdata['status'], $decoded->data->id);
    if ($response['code'] == 1) {
        $v = $postdata['status'] ==1? 'Active' : 'Inactive';
        (new Logs($db))->eventLog($decoded->data->id,EVENTS['status_cl'].' to '.$v . '  for '.$faculty->name);
