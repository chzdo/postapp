...
$t = $s->checkCurrentAdmission($session);
$v = array();
if($t){
  $state = false;
 if($s->info['app_state'] == 0){
   $state = true;
 }
$v = array("isClosed"=> $state, "dead_line"=>$s->info['dead_line'], "isAdmissionSession"=>$t);
}else{
  $v = array("isAdmissionSession"=>$t);
}
$r = $app->getSummaryAll($session);
//var_dump($r);
 
 echo sendJson(array("code"=> 1 , "message"=>"found", "payload"=>array("list"=>$r,"AdmissionSession"=>$v)));
