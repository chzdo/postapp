...
try {
    $faculty = new Session($db);


    if (!$faculty->check($postdata['id'], 1)) {
        echo sendJson(array('code' => 0, 'message' => 'Session Does not exist', 'payload' => null));
        return;
    }

    $response = $faculty->Update($postdata['id'], $postdata['name'], $decoded->data->id);
    if ($response['code'] == 1) {
        (new Logs($db))->eventLog($decoded->data->id, EVENTS['update_s'] . ' ' . $postdata['name']);
